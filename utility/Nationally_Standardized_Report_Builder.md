# Nationally Standardized Report Builder (prototype)
This is a LEAF Programmer script that functions as a Report Builder, which reports on all sites within a Nationally Standardized site collection.
...
<div id="step_2" style="display: none; width: 95%; background-color: white; border: 1px solid black; margin: 2em auto; padding: 0px">
    ...
    <div style="padding: 8px">
        ...
        <br style="clear: both" />
        <button id="generateReport" class="buttonNorm" style="position: fixed; bottom: 14px; margin: auto; left: 0; right: 0; font-size: 140%; height: 52px; padding-top: 8px; padding-bottom: 4px; width: 70%; margin: auto; text-align: center; box-shadow: 0 0 20px black">Generate Nationally Standardized Report <img src="dynicons/?img=x-office-spreadsheet-template.svg&w=32" alt="generate report" /></button>
    </div>
</div>
...
async function getSubordinateSites() {
    ...
    .then(function(res) {
        if(res.siteType != 'national_primary') {
            alert('This site is not configured as a primary national distribution site.');
            return [];
        }
        else {
