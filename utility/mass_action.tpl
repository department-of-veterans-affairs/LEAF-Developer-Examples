...
function runMassAction() {
    ...
    queue.setWorker(function(item) {    // This is executed for each record. Must return $.ajax or equivalent
        ...
        return $.ajax({
            type: 'POST',
            url: `./api/formWorkflow/${item.recordID}/apply`,
            data: {
                CSRFToken: '<!--{$CSRFToken}-->',
                dependencyID: 74,       // This is the ID for the requirement within the workflow's step
                actionType: 'AddNote',  // This is a unique ID for the action. Confirm by checking the action in the workflow: "...clicks [actionType]"
                comment: 'Moved by mass action'
            }
        });
    });
