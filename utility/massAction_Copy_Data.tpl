<script src="./files/intervalQueue2.js"></script>
<script>
var sourceFieldID = 5;
var destinationFieldID = 41;
var functionDescription = `Copy data from field#${sourceFieldID} to #${destinationFieldID} field`;
...
function runMassAction() {
    ...
    queue.setWorker(function(item) {    // This is executed for each record. Must return $.ajax or equivalent
        ...
        // check field format and handle fileuploads
        if(fieldTypes[sourceFieldID] != fieldTypes[destinationFieldID]) {
            throw new Error(`Mismatched file format between fields ${sourceFieldID}:${fieldTypes[sourceFieldID]} and ${destinationFieldID}:${fieldTypes[destinationFieldID]}`);
        }
        
        if(fieldTypes[sourceFieldID] == 'fileupload' && item.s1['id'+sourceFieldID] != '') {
            let promises = [];
            
            let files = item.s1['id'+sourceFieldID].split("\n");
            for(let i in files) {
                if(files[i] != '') {
                    promises.push(fetch(`file.php?form=${item.recordID}&id=${sourceFieldID}&series=1&file=${i}`)
                        .then(data => data.blob())
                        .then(blob => {
                            let formData = new FormData();
                            formData.append("CSRFToken", '<!--{$CSRFToken}-->');
                            formData.append(destinationFieldID, blob, files[i]);
                            return fetch(`./api/form/${item.recordID}`, {method: 'POST', body: formData});
                    }));
                }
            }
            ...
            data[destinationFieldID] = item.s1['id'+sourceFieldID];

            return $.ajax({
                type: 'POST',
                url: `./api/form/${item.recordID}`,
                data: data
            });
        }
    });
    ...
    queue.setOnWorkerError(function(item, reason) { // Errors can be logged here
        console.log(`Error processing recordID: ${item.recordID}. ${reason}`);
    });
...
function getFieldTypes() {
    return fetch('./api/form/indicator/list?x-filterData=indicatorID,format')
    	.then(res => res.json())
    	.then(data => {
    	for(let i in data) {
            fieldTypes[data[i].indicatorID] = data[i].format;
        }
    });
}
