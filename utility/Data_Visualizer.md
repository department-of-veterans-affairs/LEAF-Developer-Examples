    ...
    data.forEach(form => {
        if(form.workflowID > 0) {
            buf += `<tr>
                ...
                <td><a href="report.php?a=Data_Visualizer&form=${form.categoryID}">${form.categoryName}</a></td>
            </tr>`
        }
    });
    buf += '</table>';
...
async function getDataBuildCharts(categoryID, customSearch) {
    ...
    window.history.pushState('Loaded Report', chartTitle, 'report.php?a=Data_Visualizer&query=' + encodeURIComponent(exportQuery));    
